...
#2.0.5- World Games Edition (not included in this distrubtion)
...
#this starts the thread for the liftingcast function. 

def liftingCast_server():
    global scroll_lock_count
        
    @app.route("/")
    def index():
        return "Specify proper URL path"
    ...
    @app.route("/light_status_long")
    def light_status_long():
        return JSON.DUMPS(last_value)

    @app.route("/start_clock")
    def start_clock():
        #LiftingCast wishes to start the clock remotely.
        global scroll_lock_count, Time, clock_resp
        print 'LiftingCast Clock START Detected'
        scroll_lock_count += 1
    ...
    @app.route("/reset_clock")
    def reset_clock():
        global scroll_lock_count, Time, clock_resp, meet_break
        meet_break = False
    ...
    @app.route("/set_clock/<int:timerDuration>/")
    def set_clock(timerDuration):
        global scroll_lock_count, Time, clock_resp,meet_break
        Time[0] = timerDuration
        Time[1] = 0
        scroll_lock_count = 0
...
def change_light():
    ...
    global left_white, left_red, left_blue, left_yellow, head_white, head_red, head_blue, head_yellow, right_white, right_red, right_blue, right_yellow
    ...
    if aDown and change_mind: #left red card
        ...
        #used for liftingCast integration.
        left_white = False
        left_red = True
...
def drl_decisions_to_liftingcast_decisions(left_white,
    ...
    liftingCastLights =  {
        "statusType": "LIGHTS",
        "refLeft": drl_lights_to_decision_cards(left_white,
                                             left_red,
                                             left_blue,
                                             left_yellow),
        "refHead": drl_lights_to_decision_cards(head_white,
                                             ...
                                             head_blue,
                                             head_yellow),
        "refRight": drl_lights_to_decision_cards(right_white,
                                              right_red,
                                              right_blue,
                                              right_yellow)
    }
...
#----------------------------------------------------------------------------
           ...
           # pygame.display.update()
                ...
                drl_decisions_to_liftingcast_decisions(left_white,
                                               left_red,
                                               left_blue,
                                               left_yellow,
                                               head_white,
